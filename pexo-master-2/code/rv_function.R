...
####################################
    Tstep <- Par$Tstep
    OutBary1 <- time_Utc2tb(cbind(utc[,1],utc[,2]+Tstep),Par)
    OutTime1 <- time_Ta2te(OutBary1,Par)
    OutBary0 <- time_Utc2tb(cbind(utc[,1],utc[,2]-Tstep),Par)
    OutTime0 <- time_Ta2te(OutBary0,Par)
    dTAI <- time_T2mT2(OutBary1$JDtai,OutBary0$JDtai)
    dTDB <- time_T2mT2(OutBary1$JDtai,OutBary0$JDtdb)
    dTCB <- time_T2mT2(OutBary1$JDtai,OutBary0$JDtcb)
    dBJDtdb <- time_T2mT2(OutBary1$BJDtdb,OutBary0$BJDtdb)
    dBJDtcb <- time_T2mT2(OutBary1$BJDtcb,OutBary0$BJDtcb)
    dTT <- time_T2mT2(OutBary1$JDtai,OutBary0$JDtt)
    dTCG <- time_T2mT2(OutBary1$JDtai,OutBary0$JDtcg)
    dUT1 <- time_T2mT2(OutBary1$JDut1,OutBary0$JDut1)
...
#    ZgsO <- -(OutBary$dTCB.dTT-1)
    ZgsO <- 1/OutBary$dTCB.dTT-1
    ZlS <- ZlO <- dShapiroSolar/dTCB
    ZkS <- ZSO <- rowSums(OutTime$uOT*OutBary$SO[,4:6])/CKMPS
    ZSG <- rowSums(OutTime$uOT*OutBary$SG[,4:6])/CKMPS
    ZGO <- rowSums(OutTime$uOT*OutBary$GO[,4:6])/CKMPS
