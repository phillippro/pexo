...
assign.names <- function(par.vector,Np,p=0,q=0,n=0){
    nams <- c()
...
plot.labels.simple <- function(par.name){
    pats <- c('per','K','e','omega','Mo','a','b','beta','alpha','w','m','tau','s','c')
    new.name <- par.name
    for(pat in pats){
...
RV.kepler <- function(pars.kep,tt=NA,prior.kep=prior.type,period.kep=period.par,injection=FALSE,kep.only=FALSE,noise.only=FALSE){
    ...
    if(Np.kep>0 & !noise.only){
        pers = pars.kep[grep('per([[:digit:]]{1})',names(pars.kep))]
        ...
        for(h in 1:Np.kep){
            ...
            if(FALSE){
                dev.new()
                plot(c(tt,tt),c(dVr.p,RV.all),main=paste0('P=',P,';K=',K,';e=',e))
                points(tt,RV.all,col='red')
                Sys.sleep(2)
                dev.off()
            }
        }
        ...
        for(k in 1:Np){
            if(k!=ind.transit){
                if(period.par=='logP'){
                    ...
                    logprior  <- logprior+ log(logPprior)
                    logP = pers
                    P = exp(logP)
                }else if(period.par=='P'){
                    P = pers
                    Pprior = 1/(Pmax-Pmin)
                    logprior <- logprior+ log(Pprior)
                }else if(period.par=='nu'){
                    nv = pers
                    P = 1/nv
                    nvprior = 1/(nvmax-nvmin)
                    logprior  <- logprior+ log(nvprior)
                }
            }else{
                P <- pers[k]
                Pprior <- dnorm(P,mean=Pt,sd=ePt,log=TRUE)
                logprior <- logprior+Pprior
            }
            if(prior.type=='jeffrey'){
                Kprior = (Ks[k]+K0)^-1*(log(1+Kmax/K0*(Pmin/P)^(1/3)))^-1#according to Table 1 of Ford & Gregory 2007
            }else{
                Kprior = 1/(Kmax-Kmin)
            }
            ...
            if(k==ind.transit & !is.na(Tc)){
                Tp <- M02Tp(Mos[k],tmin,P)
                Tc.can <- calTc(Tp,P,es[k],omegas[k])
                dt <- (Tc.can-Tc)%%P
                Mo.prior <- dnorm(min(dt,abs(dt-P)),mean=0,sd=eTc,log=TRUE)
                logprior <- logprior+Mo.prior
            }else{
                logprior <- logprior+log(1/(2*pi))
            }
        }
    }
####noise model
...
#        param.new <- try(mvrnorm(n=1,mu=param,Sigma=cov.all,tol=tol1),TRUE)#this could cause some non-zero values for fix value, but it is too small to be accounted for.
        param.new <- try(mvrnorm(n=1,mu=param,Sigma=cov.all,tol=tol1),FALSE)
    ...
    if(k==Ntt){
        ...
        cat('name(param>par.max)=',names(param)[which(param>par.max)],'\n')
        cat('name(param<par.min)=',names(param)[which(param<par.min)],'\n')
        break()
