...
#library(ks)
#library(numDeriv)
###From dra ddec to offsets
dir2offset <- function(dir1,dir2){
    dra <- dir1[,1]*cos(dir1[,2])-dir2[,1]*cos(dir2[,2])
    ddec <- dir1[,2]-dir2[,2]
    cbind(dra,ddec)
}
...
ele2bl <- function(oe){
   ...
   for(i in 1:N){
      ...
      CS <- oe2cs(OE)
      bl <- xyz2bl.vec(t(CS))#its ecliptic coord.: lambda, beta
...
#       cat('CS=',CS,'\n')
       betalambda.pos <- xyz2bl.vec(t(CS[1:3]))#its ecliptic coord.: lambda, beta
       betalambda.vel <- xyz2bl.vec(t(CS[4:6]))#
...
ele2ecl <- function(oe){
   ...
   for(i in 1:N){
       ...
       Rnorm <- sqrt(CS[1:3]%*%CS[1:3])
       Vnorm <- sqrt(CS[4:6]%*%CS[4:6])
       bl.pos <- xyz2bl.vec(t(CS[1:3]))#its ecliptic coord.: lambda, beta
       bl.vel <- xyz2bl.vec(t(CS[4:6]))#
...
###convert time-varying states back to observables
    dec.ra <- xyz2bl.vec(cbind(x1,y1,z1))
...
#updated function
state.g2e <- function(state,xyzuvw=TRUE){
    auyr2kms <- 4.74047
    kpcmyr2auyr <- 1e3*206265/1e6
    kpcmyr2kms <- kpcmyr2auyr*auyr2kms
...
deg2hdms <- function(rd){
    RAdeg <- rd[,1,drop=FALSE]
    DEdeg <- rd[,2,drop=FALSE]
...
hdms2deg <- function(rah,ram,ras,ded,dem,des){
    ...
    cbind(RA.deg=RA.deg,DEC.deg=DEC.deg,RA.rad=RA.rad,DEC.rad=DEC.rad)
}

xyz2hdms <- function(xyz){
    da <- xyz2bl(xyz[,1,drop=FALSE],xyz[,2,drop=FALSE],xyz[,3,drop=FALSE])*180/pi#deg
    deg2hdms(da[,2:1,drop=FALSE])
}
