...
usage () {
    ...
    echo
    echo "-P, --Planet       Planet number [optional, default=NA]"
    echo
    echo "-O, --orbit        Initial orbital parameters [optional, default=NULL]"
    echo
...
if [ ! -z "$Planet" ]
then
    command="$command -P $Planet"
fi

if [ ! -z "$orbit" ]
then
    command="$command -o $orbit"
fi
